...
const Chat = {
            ...
            if (msg.text.includes('```')) {
                ...
                parts.forEach((part, i) => {
                    if (i % 2 === 1) {
                        ...
                        // Only show "Review Changes" for AI messages
                        if (msg.role === 'ai') {
    ...
    handleReviewChanges(newCode) {
        ...
        if (selection && !selection.isEmpty()) {
            // If user has selection, replace only that part
            const range = selection;
            const model = Editor.editor.getModel();
            
            // Create a preview of what the full file would look like
