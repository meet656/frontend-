...
const Chat = {
    ...
    attachEventListeners() {
        ...
        if (toggleHistoryBtn) {
            toggleHistoryBtn.onclick = () => {
                ...
                // Adjust panel width when history is visible
                if (!sidebar.classList.contains('hidden')) {
                    aiPanel.style.width = '600px'; // Expand to fit history + chat
                } else {
                    aiPanel.style.width = '400px'; // Restore normal width
                }
                
                if (Editor.editor) setTimeout(() => Editor.editor.layout(), 300);
            };
        }
        ...
        document.addEventListener('click', () => {
            if (uploadMenu) uploadMenu.classList.add('hidden');
        });

        const uploadFileOpt = document.getElementById('upload-file-option');
        if (uploadFileOpt) {
            uploadFileOpt.onclick = () => {
                fileInput.click();
            };
        }

        const uploadFolderOpt = document.getElementById('upload-folder-option');
        if (uploadFolderOpt) {
            uploadFolderOpt.onclick = () => {
                folderInput.click();
            };
        }

        if (fileInput) fileInput.onchange = (e) => this.handleFileUpload(e.target.files);
        if (folderInput) folderInput.onchange = (e) => this.handleFileUpload(e.target.files);
    ...
    renderMessages() {
        ...
        // Improved Auto-scroll with small delay to ensure content is rendered
        setTimeout(() => {
