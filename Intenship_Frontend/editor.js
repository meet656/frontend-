/**
 * Editor Logic
 * Handles Monaco Editor, Tabs, and Advanced File System Simulation
 */

const Editor = {
    editor: null,
    diffEditor: null,
    fileSystem: [], // Array of objects: { id, name, type: 'file'|'folder', parentId, content, language, isOpen: boolean, model: monaco.editor.ITextModel }
    activeFileId: null,
    openTabs: [],
    currentFolder: null,
    originalModel: null,
    modifiedModel: null,
    autoSaveEnabled: false,
    autoSaveTimer: null,

    init() {
        this.loadFileSystem();
        this.setupMonaco();
        this.renderFileTree();
        this.attachEventListeners();
        this.attachShortcuts();
        this.updateEmptyState();
        this.setupDiffEditorHandlers();
        this.initAutoSave();
    },

    initAutoSave() {
        const saved = localStorage.getItem('editor_auto_save') === 'true';
        this.autoSaveEnabled = saved;
        this.updateAutoSaveUI();
    },

    toggleAutoSave() {
        this.autoSaveEnabled = !this.autoSaveEnabled;
        localStorage.setItem('editor_auto_save', this.autoSaveEnabled);
        this.updateAutoSaveUI();
        Utils.showToast(`Auto Save ${this.autoSaveEnabled ? 'Enabled' : 'Disabled'}`, 'info');
    },

    updateAutoSaveUI() {
        const check = document.getElementById('auto-save-check');
        if (check) {
            check.innerHTML = this.autoSaveEnabled ? '✓' : '';
        }
    },

    attachShortcuts() {
        window.addEventListener('keydown', (e) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                e.preventDefault();
                this.saveFile(this.activeFileId);
            }
            if ((e.ctrlKey || e.metaKey) && e.key === 'w') {
                e.preventDefault();
                if (this.activeFileId) this.closeTab(this.activeFileId);
            }
        });
    },

    setupDiffEditorHandlers() {
        document.getElementById('accept-diff-btn').onclick = () => this.acceptChanges();
        document.getElementById('reject-diff-btn').onclick = () => this.rejectChanges();
    },

    openDiffView(originalContent, modifiedContent) {
        const overlay = document.getElementById('diff-editor-overlay');
        overlay.classList.remove('hidden');

        if (!this.diffEditor) {
            this.diffEditor = monaco.editor.createDiffEditor(document.getElementById('monaco-diff-container'), {
                theme: Settings.theme === 'light' ? 'vs' : 'vs-dark',
                automaticLayout: true,
                originalEditable: false,
                renderSideBySide: true
            });
        }

        const activeFile = this.fileSystem.find(f => f.id === this.activeFileId);
        const language = activeFile ? activeFile.language : 'javascript';

        this.originalModel = monaco.editor.createModel(originalContent, language);
        this.modifiedModel = monaco.editor.createModel(modifiedContent, language);

        this.diffEditor.setModel({
            original: this.originalModel,
            modified: this.modifiedModel
        });
    },

    acceptChanges() {
        if (!this.modifiedModel || !this.activeFileId) return;
        
        const newContent = this.modifiedModel.getValue();
        const file = this.fileSystem.find(f => f.id === this.activeFileId);
        
        if (file) {
            file.content = newContent;
            file.isDirty = false;
            if (file.model) {
                file.model.setValue(newContent);
            }
            this.saveFileSystem();
            this.renderTabs();
        }

        this.closeDiffView();
        Utils.showToast('Changes applied', 'success');
    },

    rejectChanges() {
        this.closeDiffView();
        Utils.showToast('Changes discarded', 'info');
    },

    closeDiffView() {
        const overlay = document.getElementById('diff-editor-overlay');
        overlay.classList.add('hidden');

        if (this.originalModel) this.originalModel.dispose();
        if (this.modifiedModel) this.modifiedModel.dispose();
        
        this.originalModel = null;
        this.modifiedModel = null;
    },

    loadFileSystem() {
        const saved = localStorage.getItem('app_file_system');
        if (saved) {
            this.fileSystem = JSON.parse(saved);
        } else {
            // Default structure
            this.fileSystem = [
                { id: 'root_src', name: 'src', type: 'folder', parentId: null, isOpen: true },
                { id: '1', name: 'index.js', type: 'file', parentId: 'root_src', content: '// Welcome\nconsole.log("Hello!");', language: 'javascript' },
                { id: '2', name: 'styles.css', type: 'file', parentId: 'root_src', content: 'body { color: white; }', language: 'css' },
                { id: '3', name: 'index.html', type: 'file', parentId: null, content: '<html></html>', language: 'html' }
            ];
            this.saveFileSystem();
        }
    },

    saveFileSystem() {
        const serializedFS = this.fileSystem.map(item => {
            const { model, handle, ...serializableItem } = item;
            return serializableItem;
        });
        localStorage.setItem('app_file_system', JSON.stringify(serializedFS));
    },

    setupMonaco() {
        require.config({ paths: { vs: 'https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.44.0/min/vs' } });
        require(['vs/editor/editor.main'], () => {
            const currentTheme = Settings.theme === 'light' ? 'vs' : 'vs-dark';
            monaco.editor.setTheme(currentTheme);

            this.editor = monaco.editor.create(document.getElementById('monaco-editor-container'), {
                theme: currentTheme,
                automaticLayout: true,
                fontSize: (typeof Settings !== 'undefined') ? Settings.fontSize : 14,
                tabSize: (typeof Settings !== 'undefined') ? Settings.tabSize : 4,
                lineHeight: 22,
                padding: { top: 16, bottom: 16 }
            });

            this.editor.onDidChangeCursorPosition(e => {
                document.getElementById('editor-cursor-pos').textContent = `Ln ${e.position.lineNumber}, Col ${e.position.column}`;
            });

            // Restore last active file if any
            const lastActive = localStorage.getItem('last_active_file');
            if (lastActive && this.fileSystem.find(f => f.id === lastActive)) {
                this.openFile(lastActive);
            }
        });
    },

    renderFileTree() {
        console.log('Rendering File Tree... Current State:', this.fileSystem);
        const container = document.getElementById('file-tree');
        if (!container) return;
        
        container.innerHTML = '';
        
        const renderItem = (item, level = 0) => {
            const div = document.createElement('div');
            div.className = `file-item ${item.id === this.activeFileId ? 'active' : ''} ${item.type}`;
            div.style.paddingLeft = `${level * 16 + 12}px`;
            div.draggable = true;
            
            // Icon handling
            let icon = '';
            if (item.type === 'folder') {
                const arrow = item.isOpen ? '▾' : '▸';
                const folderIcon = item.isOpen ? '📂' : '📁';
                icon = `<span class="arrow">${arrow}</span> <span class="folder-icon">${folderIcon}</span>`;
            } else {
                icon = `<span class="file-icon-inner">${this.getFileIcon(item.name)}</span>`;
            }

            div.innerHTML = `
                <span class="file-icon">${icon}</span>
                <span class="file-name">${item.name}</span>
                <div class="file-actions">
                    <button onclick="event.stopPropagation(); Editor.renameItem('${item.id}')" title="Rename">✎</button>
                    <button onclick="event.stopPropagation(); Editor.deleteItem('${item.id}')" title="Delete">✕</button>
                </div>
            `;

            div.onclick = (e) => {
                e.stopPropagation();
                if (item.type === 'folder') {
                    item.isOpen = !item.isOpen;
                    this.saveFileSystem();
                    this.renderFileTree();
                } else {
                    this.openFile(item.id);
                }
            };

            // Drag and drop logic (kept existing)
            div.ondragstart = (e) => {
                e.dataTransfer.setData('text/plain', item.id);
                div.classList.add('dragging');
                document.body.classList.add('dragging-mode');
            };

            div.ondragend = () => {
                div.classList.remove('dragging');
                document.body.classList.remove('dragging-mode');
            };

            div.ondragover = (e) => {
                e.preventDefault();
                div.classList.add('drag-over');
            };

            div.ondragleave = () => {
                div.classList.remove('drag-over');
            };

            div.ondrop = (e) => {
                e.preventDefault();
                const draggedId = e.dataTransfer.getData('text/plain');
                if (draggedId !== item.id) {
                    this.moveItem(draggedId, item.id);
                }
            };

            container.appendChild(div);

            if (item.type === 'folder' && item.isOpen) {
                const children = this.fileSystem.filter(f => f.parentId === item.id)
                    .sort((a, b) => (a.type === b.type ? a.name.localeCompare(b.name) : a.type === 'folder' ? -1 : 1));
                children.forEach(child => renderItem(child, level + 1));
            }
        };

        const rootItems = this.fileSystem.filter(f => f.parentId === null)
            .sort((a, b) => (a.type === b.type ? a.name.localeCompare(b.name) : a.type === 'folder' ? -1 : 1));
        
        rootItems.forEach(item => renderItem(item, 0));
        console.log('File Tree Rendered Successfully');
    },

    getFileIcon(name) {
        if (name.endsWith('.js')) return 'JS';
        if (name.endsWith('.html')) return '<>';
        if (name.endsWith('.css')) return '#';
        return '📄';
    },

    getLanguage(filename) {
        if (filename.endsWith(".js")) return "javascript";
        if (filename.endsWith(".py")) return "python";
        if (filename.endsWith(".html")) return "html";
        if (filename.endsWith(".css")) return "css";
        return "plaintext";
    },

    openFile(id) {
        const file = this.fileSystem.find(f => f.id === id);
        if (!file || file.type === 'folder') return;

        this.activeFileId = id;
        localStorage.setItem('last_active_file', id);

        if (this.editor) {
            // Use Model-Based Approach: If model doesn't exist, create it
            if (!file.model) {
                const language = this.getLanguage(file.name);
                file.model = monaco.editor.createModel(file.content, language);
                
                // Track changes per model to keep track of dirty state
                file.model.onDidChangeContent(() => {
                    if (!file.isDirty) {
                        file.isDirty = true;
                        this.renderTabs();
                    }

                    // Auto Save Logic
                    if (this.autoSaveEnabled) {
                        if (this.autoSaveTimer) clearTimeout(this.autoSaveTimer);
                        this.autoSaveTimer = setTimeout(() => {
                            this.saveFile(file.id, true);
                        }, 1000);
                    }
                });
            }

            // Set the model to the editor (This enables syntax highlighting)
            this.editor.setModel(file.model);
            
            // Ensure layout updates
            setTimeout(() => this.editor.layout(), 0);
        }

        if (!this.openTabs.includes(id)) {
            this.openTabs.push(id);
        }

        document.getElementById('current-file-name').textContent = file.name;
        document.getElementById('editor-lang').textContent = this.getLanguage(file.name).toUpperCase();
        
        this.renderTabs();
        this.renderFileTree();
        this.updateEmptyState();
    },

    saveFile(id, isAutoSave = false) {
        const file = this.fileSystem.find(f => f.id === id);
        if (!file || !file.model) return;

        const newContent = file.model.getValue();
        file.content = newContent;
        file.isDirty = false;

        // Optional: Save to real file system if handle exists
        if (file.handle) {
            this.saveToRealFileSystem(file.handle, newContent);
        }

        this.saveFileSystem();
        this.renderTabs();

        if (!isAutoSave) {
            Utils.showToast(`Saved ${file.name}`, 'success');
        }
    },

    async saveToRealFileSystem(handle, content) {
        try {
            const writable = await handle.createWritable();
            await writable.write(content);
            await writable.close();
            console.log('Saved to real file system');
        } catch (err) {
            console.error('Failed to save to real file system:', err);
            Utils.showToast('Could not save to disk', 'error');
        }
    },

    moveItem(draggedId, targetId) {
        const dragged = this.fileSystem.find(i => i.id === draggedId);
        const target = this.fileSystem.find(i => i.id === targetId);
        
        if (!dragged || !target) return;
        
        // Prevent moving into itself
        if (draggedId === targetId) return;
        
        // Prevent moving a folder into its own descendants
        const isDescendant = (parentId, childId) => {
            if (parentId === childId) return true;
            const child = this.fileSystem.find(i => i.id === childId);
            if (!child || !child.parentId) return false;
            return isDescendant(parentId, child.parentId);
        };

        if (dragged.type === 'folder' && isDescendant(draggedId, targetId)) {
            Utils.showToast("Cannot move a folder into itself", "error");
            return;
        }
        
        // Move to new parent
        if (target.type === 'folder') {
            dragged.parentId = target.id;
        } else {
            dragged.parentId = target.parentId;
        }
        
        this.saveFileSystem();
        this.renderFileTree();
        Utils.showToast(`Moved ${dragged.name}`, 'success');
    },

    renderTabs() {
        const bar = document.getElementById('tabs-bar');
        bar.innerHTML = '';
        this.openTabs.forEach(id => {
            const file = this.fileSystem.find(f => f.id === id);
            if (!file) return;
            const tab = document.createElement('div');
            tab.className = `tab ${id === this.activeFileId ? 'active' : ''} ${file.isDirty ? 'dirty' : ''}`;
            tab.draggable = true;
            
            tab.ondragstart = (e) => {
                e.dataTransfer.setData('text/tab', id);
                tab.classList.add('dragging');
            };
            
            tab.ondragover = (e) => {
                e.preventDefault();
                tab.classList.add('drag-over');
            };
            
            tab.ondragleave = () => tab.classList.remove('drag-over');
            
            tab.ondrop = (e) => {
                e.preventDefault();
                const draggedTabId = e.dataTransfer.getData('text/tab');
                if (draggedTabId && draggedTabId !== id) {
                    const fromIdx = this.openTabs.indexOf(draggedTabId);
                    const toIdx = this.openTabs.indexOf(id);
                    this.openTabs.splice(fromIdx, 1);
                    this.openTabs.splice(toIdx, 0, draggedTabId);
                    this.renderTabs();
                }
            };
            
            tab.ondragend = () => tab.classList.remove('dragging');

            tab.innerHTML = `
                ${file.isDirty ? '<span class="unsaved-indicator">●</span>' : ''}
                <span>${file.name}</span>
                <span class="tab-close" onclick="event.stopPropagation(); Editor.closeTab('${id}')">✕</span>
            `;
            tab.onclick = () => this.openFile(id);
            bar.appendChild(tab);
        });
    },

    closeTab(id) {
        this.openTabs = this.openTabs.filter(tid => tid !== id);
        if (this.activeFileId === id) {
            this.activeFileId = this.openTabs.length > 0 ? this.openTabs[this.openTabs.length - 1] : null;
            if (this.activeFileId) {
                this.openFile(this.activeFileId);
            } else {
                // No more tabs open
                if (this.editor) {
                    this.editor.setModel(null);
                }
                document.getElementById('current-file-name').textContent = 'No File Open';
                document.getElementById('editor-lang').textContent = '';
            }
        }
        this.renderTabs();
        this.renderFileTree();
        this.updateEmptyState();
    },

    closeFile() {
        if (this.activeFileId) {
            this.closeTab(this.activeFileId);
            Utils.showToast('File closed');
        } else {
            Utils.showToast('No active file to close', 'info');
        }
    },

    closeFolder() {
        if (this.fileSystem.length === 0) {
            Utils.showToast('No folder open', 'info');
            return;
        }
        
        if (confirm('Are you sure you want to close the current folder?')) {
            // Dispose all models
            this.fileSystem.forEach(item => {
                if (item.model) item.model.dispose();
            });
            
            this.fileSystem = [];
            this.openTabs = [];
            this.activeFileId = null;
            this.currentFolder = null;
            
            if (this.editor) this.editor.setModel(null);
            
            this.saveFileSystem();
            this.renderFileTree();
            this.renderTabs();
            this.updateEmptyState();
            
            Utils.showToast('Folder closed');
        }
    },

    updateEmptyState() {
        const emptyState = document.getElementById('empty-state');
        const editorContainer = document.getElementById('monaco-editor-container');
        if (this.activeFileId) {
            emptyState.classList.add('hidden');
            editorContainer.style.display = 'block';
        } else {
            emptyState.classList.remove('hidden');
            editorContainer.style.display = 'none';
        }
    },

    // --- New File/Folder Handling Logic ---

    async handleDrop(items) {
        Utils.showToast('Reading dropped items...', 'info');
        let firstFileId = null;
        for (let i = 0; i < items.length; i++) {
            const item = items[i].webkitGetAsEntry();
            if (item) {
                const id = await this.traverseFileTree(item);
                if (!firstFileId && id && id.startsWith('file_')) {
                    firstFileId = id;
                }
            }
        }
        this.saveFileSystem();
        this.renderFileTree();
        this.updateEmptyState();
        
        if (firstFileId) {
            this.openFile(firstFileId);
        }
        
        Utils.showToast('Upload complete', 'success');
    },

    async handleInputUpload(files, isFolder) {
        Utils.showToast('Uploading...', 'info');
        let firstFileId = null;
        for (let i = 0; i < files.length; i++) {
            const file = files[i];
            const path = file.webkitRelativePath || file.name;
            const parts = path.split('/');
            
            let parentId = null;
            // If it's a folder upload, recreate the directory structure
            if (isFolder && parts.length > 1) {
                for (let j = 0; j < parts.length - 1; j++) {
                    const folderName = parts[j];
                    let folder = this.fileSystem.find(f => f.name === folderName && f.parentId === parentId && f.type === 'folder');
                    if (!folder) {
                        folder = {
                            id: 'folder_' + Math.random().toString(36).substr(2, 9),
                            name: folderName,
                            type: 'folder',
                            parentId: parentId,
                            isOpen: true
                        };
                        this.fileSystem.push(folder);
                    }
                    parentId = folder.id;
                }
            }

            const content = await this.readFileContent(file);
            const fileName = parts[parts.length - 1];
            const fileId = 'file_' + Math.random().toString(36).substr(2, 9);
            this.fileSystem.push({
                id: fileId,
                name: fileName,
                type: 'file',
                parentId: parentId,
                content: content,
                language: this.getLanguage(fileName)
            });
            if (!firstFileId) firstFileId = fileId;
        }
        this.saveFileSystem();
        this.renderFileTree();
        this.updateEmptyState();
        
        if (firstFileId) {
            this.openFile(firstFileId);
        }
        
        Utils.showToast('Upload complete', 'success');
    },

    async traverseFileTree(item, parentId = null) {
        const id = (item.isDirectory ? 'folder_' : 'file_') + Math.random().toString(36).substr(2, 9);
        
        if (item.isFile) {
            const file = await new Promise((resolve) => item.file(resolve));
            const content = await this.readFileContent(file);
            this.fileSystem.push({
                id: id,
                name: item.name,
                type: 'file',
                parentId: parentId,
                content: content,
                language: this.getLanguage(item.name)
            });
            return id;
        } else if (item.isDirectory) {
            this.fileSystem.push({
                id: id,
                name: item.name,
                type: 'folder',
                parentId: parentId,
                isOpen: true
            });
            
            const dirReader = item.createReader();
            const entries = await new Promise((resolve) => {
                dirReader.readEntries(resolve);
            });
            
            let firstId = null;
            for (let i = 0; i < entries.length; i++) {
                const childId = await this.traverseFileTree(entries[i], id);
                if (!firstId) firstId = childId;
            }
            return firstId || id;
        }
        return null;
    },

    readFileContent(file) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
            reader.onload = (e) => resolve(e.target.result);
            reader.onerror = (e) => reject(e);
            reader.readAsText(file);
        });
    },

    addNewFile() {
        const name = prompt('File name:');
        if (!name) return;
        
        const parentId = null;
        if (this.fileSystem.some(f => f.name === name && f.parentId === parentId && f.type === 'file')) {
            Utils.showToast(`File "${name}" already exists`, 'error');
            return;
        }
        
        console.log('Creating new file:', name);
        const id = 'file_' + Date.now();
        const lang = this.getLanguage(name);
        
        const newFile = { id, name, type: 'file', parentId, content: '', language: lang };
        this.fileSystem.push(newFile);
        
        this.saveFileSystem();
        this.renderFileTree();
        this.openFile(id);
        
        Utils.showToast(`File "${name}" created`, 'success');
        if (window.Terminal) Terminal.logToOutput(`New file created: ${name}`, 'success');
    },

    addNewFolder() {
        const name = prompt('Folder name:');
        if (!name) return;
        
        const parentId = null;
        if (this.fileSystem.some(f => f.name === name && f.parentId === parentId && f.type === 'folder')) {
            Utils.showToast(`Folder "${name}" already exists`, 'error');
            return;
        }
        
        console.log('Creating new folder:', name);
        const id = 'folder_' + Date.now();
        const newFolder = { id, name, type: 'folder', parentId, isOpen: true };
        this.fileSystem.push(newFolder);
        
        this.saveFileSystem();
        this.renderFileTree();
        
        Utils.showToast(`Folder "${name}" created`, 'success');
        if (window.Terminal) Terminal.logToOutput(`New folder created: ${name}`, 'success');
    },

    renameItem(id) {
        const item = this.fileSystem.find(i => i.id === id);
        if (!item) return;
        
        const newName = prompt('New name:', item.name);
        if (newName && newName !== item.name) {
            console.log(`Renaming ${item.name} to ${newName}`);
            item.name = newName;
            
            // If it's a file with a model, we might need to update model uri if we were using uris, 
            // but here we just use the name in the UI.
            
            this.saveFileSystem();
            this.renderFileTree();
            
            if (this.activeFileId === id) {
                document.getElementById('current-file-name').textContent = newName;
            }
            
            Utils.showToast('Renamed successfully');
        }
    },

    deleteItem(id) {
        if (!confirm('Are you sure you want to delete this?')) return;
        
        // Recursive helper to get all child IDs
        const getChildrenIds = (parentId) => {
            let ids = [];
            const children = this.fileSystem.filter(i => i.parentId === parentId);
            children.forEach(child => {
                ids.push(child.id);
                if (child.type === 'folder') {
                    ids = ids.concat(getChildrenIds(child.id));
                }
            });
            return ids;
        };

        const idsToDelete = [id, ...getChildrenIds(id)];

        idsToDelete.forEach(deleteId => {
            const item = this.fileSystem.find(i => i.id === deleteId);
            if (item && item.model) {
                item.model.dispose();
            }
            this.closeTab(deleteId); // Safe to call even if not opened
        });

        this.fileSystem = this.fileSystem.filter(i => !idsToDelete.includes(i.id));
        this.saveFileSystem();
        this.renderFileTree();
        Utils.showToast('Item deleted', 'error');
    },

    async openProject() {
        try {
            const dirHandle = await window.showDirectoryPicker();
            this.closeProject(); // Clear current project
            
            this.currentFolder = dirHandle.name;
            localStorage.setItem('last_project_name', dirHandle.name);
            
            await this.readDirectoryRecursively(dirHandle);
            this.saveFileSystem();
            this.renderFileTree();
            Utils.showToast(`Project "${dirHandle.name}" opened`, 'success');
        } catch (err) {
            if (err.name !== 'AbortError') {
                console.error('Failed to open project:', err);
                Utils.showToast('Failed to open project', 'error');
            }
        }
    },

    async readDirectoryRecursively(dirHandle, parentId = null) {
        for await (const entry of dirHandle.values()) {
            const id = `${parentId ? parentId + '_' : ''}${entry.name}`;
            
            if (entry.kind === 'file') {
                const file = await entry.getFile();
                const content = await file.text();
                this.fileSystem.push({
                    id,
                    name: entry.name,
                    type: 'file',
                    parentId,
                    content,
                    language: this.getLanguage(entry.name),
                    handle: entry // Store handle for saving
                });
            } else if (entry.kind === 'directory') {
                this.fileSystem.push({
                    id,
                    name: entry.name,
                    type: 'folder',
                    parentId,
                    isOpen: false
                });
                await this.readDirectoryRecursively(entry, id);
            }
        }
    },

    closeProject() {
        // Dispose all models
        this.fileSystem.forEach(item => {
            if (item.model) item.model.dispose();
        });
        
        this.fileSystem = [];
        this.openTabs = [];
        this.activeFileId = null;
        this.currentFolder = null;
        localStorage.removeItem('app_file_system');
        localStorage.removeItem('last_project_name');
        
        if (this.editor) this.editor.setModel(null);
        
        this.renderFileTree();
        this.renderTabs();
        this.updateEmptyState();
        Utils.showToast('Project closed', 'info');
    },

    attachEventListeners() {
        // Project Buttons
        const openProjectBtn = document.querySelector('.activity-icon[title="Open Project"]');
        if (openProjectBtn) {
            openProjectBtn.onclick = () => this.openProject();
        }

        const closeProjectBtn = document.querySelector('.activity-icon[title="Close Project"]');
        if (closeProjectBtn) {
            closeProjectBtn.onclick = () => this.closeProject();
        }

        document.getElementById('new-file-btn').onclick = () => this.addNewFile();
        document.getElementById('new-folder-btn').onclick = () => this.addNewFolder();
        
        // save-btn handled below
        
        // --- Upload Handlers ---
        const fileInput = document.getElementById('upload-file-input');
        const folderInput = document.getElementById('upload-folder-input');

        const triggerFileUpload = () => fileInput.click();
        const triggerFolderUpload = () => folderInput.click();

        // Bind all upload buttons
        ['menu-upload-file', 'empty-upload-file-btn'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.onclick = triggerFileUpload;
        });
        ['menu-upload-folder', 'empty-upload-folder-btn'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.onclick = triggerFolderUpload;
        });

        fileInput.onchange = (e) => this.handleInputUpload(e.target.files, false);
        folderInput.onchange = (e) => this.handleInputUpload(e.target.files, true);

        // --- Drag & Drop Handlers ---
        const dropZone = document.getElementById('drop-zone');
        const dropOverlay = document.getElementById('drop-overlay');

        if (dropZone) {
            window.addEventListener('dragover', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (dropOverlay) dropOverlay.classList.add('active');
            });

            window.addEventListener('dragleave', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (e.relatedTarget === null) {
                    if (dropOverlay) dropOverlay.classList.remove('active');
                }
            });

            window.addEventListener('drop', async (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (dropOverlay) dropOverlay.classList.remove('active');
                
                const items = e.dataTransfer.items;
                if (items) {
                    await this.handleDrop(items);
                }
            });
        }

        // --- File System Access API Handlers (Existing) ---
        const openFileAction = async () => {
            try {
                const [fileHandle] = await window.showOpenFilePicker();
                const file = await fileHandle.getFile();
                const content = await file.text();
                const id = 'open_' + Date.now();
                const lang = this.getLanguage(file.name);
                this.fileSystem.push({ 
                    id, 
                    name: file.name, 
                    type: 'file', 
                    parentId: null, 
                    content, 
                    language: lang,
                    handle: fileHandle // Store handle for saving
                });
                this.saveFileSystem();
                this.renderFileTree();
                this.openFile(id);
                Utils.showToast(`Opened ${file.name}`);
            } catch (err) {
                console.error(err);
            }
        };

        const openFolderAction = async () => {
            try {
                const directoryHandle = await window.showDirectoryPicker();
                console.log('Opening Folder:', directoryHandle.name);
                
                this.fileSystem = [];
                this.openTabs = [];
                this.activeFileId = null;
                this.currentFolder = directoryHandle.name;
                
                const processHandle = async (handle, parentId = null) => {
                    const id = 'item_' + Math.random().toString(36).substr(2, 9);
                    
                    if (handle.kind === 'directory') {
                        this.fileSystem.push({
                            id,
                            name: handle.name,
                            type: 'folder',
                            parentId,
                            isOpen: true
                        });
                        
                        for await (const entry of handle.values()) {
                            await processHandle(entry, id);
                        }
                    } else {
                        const file = await handle.getFile();
                        const content = await file.text();
                        const lang = this.getLanguage(file.name);
                        
                        this.fileSystem.push({
                            id,
                            name: file.name,
                            type: 'file',
                            parentId,
                            content,
                            language: lang,
                            handle: handle // Store handle for saving
                        });
                    }
                };

                await processHandle(directoryHandle);
                
                this.saveFileSystem();
                this.renderFileTree();
                this.renderTabs();
                this.updateEmptyState();
                
                Utils.showToast(`Folder "${directoryHandle.name}" loaded`, 'success');
                if (window.Terminal) Terminal.logToOutput(`Folder loaded: ${directoryHandle.name}`, 'success');
            } catch (err) {
                console.error('Error opening folder:', err);
                if (err.name !== 'AbortError') Utils.showToast('Failed to open folder', 'error');
            }
        };

        document.getElementById('open-file-btn').onclick = openFileAction;
        document.getElementById('menu-open-file').onclick = openFileAction;
        if (document.getElementById('empty-open-file-btn')) {
            document.getElementById('empty-open-file-btn').onclick = openFileAction;
        }

        document.getElementById('open-folder-btn').onclick = openFolderAction;
        document.getElementById('menu-open-folder').onclick = openFolderAction;
        if (document.getElementById('empty-open-folder-btn')) {
            document.getElementById('empty-open-folder-btn').onclick = openFolderAction;
        }

        document.getElementById('save-btn').onclick = () => {
            if (this.activeFileId) {
                this.saveFile(this.activeFileId);
            } else {
                this.saveFileSystem();
                Utils.showToast('Project state saved', 'info');
            }
        };
        document.getElementById('run-btn').onclick = () => {
            const fileName = this.fileSystem.find(f => f.id === this.activeFileId)?.name || 'code';
            const file = this.fileSystem.find(f => f.id === this.activeFileId);
            
            if (!file) return;

            // Log to terminal/output
            if (window.Terminal) {
                Terminal.logToOutput(`Running ${fileName}...`, 'info');
                Terminal.switchPanel('output');
            }

            if (file.language === 'javascript') {
                try {
                    // Execute code safely using eval
                    // We wrap it in an IIFE to keep the scope clean
                    const code = file.content;
                    eval(`(function() { ${code} \n})()`);

                    if (window.Terminal) {
                        Terminal.logToOutput(`Execution finished.`, 'success');
                        Terminal.logToDebug(`--- Execution End ---`, 'success');
                    }
                } catch (err) {
                    if (window.Terminal) {
                        Terminal.logToOutput(`Execution failed: ${err.message}`, 'error');
                        Terminal.logToDebug(`Error: ${err.message}`, 'error');
                    }
                    console.error('Execution Error:', err);
                }
            } else {
                if (window.Terminal) {
                    Terminal.logToOutput(`Language '${file.language}' execution not supported.`, 'warn');
                }
            }
        };
    }
};
